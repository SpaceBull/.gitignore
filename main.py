...
def start(message):
    ...
    markup.add(menu_button_profile, menu_button_help, menu_button_change, menu_button_top)
    user_id = message.from_user.id
    username = str(message.from_user.username)  # - никнейм
    database.start(user_id, username)
...
def get_text_messages(message):
    if message.text == '\U0001F464Профиль':
        ...
        bot.send_message(message.from_user.id,
                         ...
                         '\n├ Ваш id: ' + id_user +
                         '\n└ Ваш баланс в USDT: ' + str(database.balance(id_user)[0][0]) +
    ...
    if message.text == '\U0001F4B8Обменник':
        ...
        text = f'*Монета \U0001FA99 | Цена \U0001F4B2 | *'
        bot.send_message(message.chat.id, text, parse_mode='Markdown')
        roster_coin = '\n'.join([f'{str(coin[1])}    {str(coin[2])}$' for coin in database.quote()])
        ...
        bot.send_message(message.chat.id, text, parse_mode='Markdown')
        roster_top = '\n'.join([f'{str(index+1)}.   {str(column[1]): ^15}  {str(column[2])}$'
                                for index, column in enumerate(database.top_person())])
        ...
        balance_wallet = database.balance_wallet(message.chat.id)
        if balance_wallet:
            bot.send_message(message.chat.id, text, parse_mode='Markdown')
            roster_wallet_user = '\n'.join([f'{str(column[0])}.   {str(column[1]): ^15}  {str(column[2])}$'
                                            for column in database.balance_wallet(message.chat.id)])
        ...
        text = f'Монета\U0001F3F7| Количество монет\U0001F4B0| Цена\U0001F9EE|'
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        balance = database.balance_wallet(message.chat.id)  # Получение списка монет в кошельке пользователя
        ...
        roster_coin = '\n'.join([f'{str(coin[1])}    {str(coin[2])}$' for coin in database.quote()])
        bot.send_message(message.chat.id, str(roster_coin))
        bot.send_message(message.chat.id, 'Выберите токен для покупки в меню',
                         reply_markup=markup)

    elif message.text == 'Cosmos':
        count_money = bot.send_message(message.chat.id, text_input)
        bot.register_next_step_handler(count_money, lambda count: calculate_price_buy(message.text,
...
def calculate_price_buy(name_coin, user_id, number_of_coins_to_buy, number_token):  # Считает покупку монет
    price_coin = database.price_token(name_coin)  # берет из БД цену за единицу токена
    ...
    balance = database.balance(user_id)[0][0]
    if result_price_coin < balance:  # сравнивает желаемую сумму приобретаемых токенов с своим балансом
        ...
        bot.send_message(user_id, f'Поздравляем с покупкой {name_coin}!\n'
        ...
        database.load_to_wallet(str(user_id), str(number_token), str(number_of_coins_to_buy.text))  # загружает id юзера
        # + id монеты и количество монет в кошелек (БД Wallet)
        database.balance_update(str(remains), str(user_id))  # обновляет баланс персонажа
    else:
        bot.send_message(user_id, 'У Вас недостаточно средств, попробуй ввести поменьше количество монет'
                                  ' или продай свои токены для пополнения USDT')


def check_balance(message, count_money, id_money):  # Сверяет желаемое продать и то, что есть в наличии у пользователя.
    count_money_input_user = database.count_money_user(message.chat.id, id_money)
...
def calculate_price_sell(name_coin, user_id, id_token, remains_money_user, want_sell_count_token):
    ...
    price_coin = database.price_token(name_coin)  # берет из БД актуальную цену на токен
    ...
    current = database.balance(user_id)  # Запрашивает текущий баланс
    new_balance = int(current[0][0]) + result_price  # Складывает текущий баланс с проданными монетами
    database.balance_update(new_balance, user_id)  # загружает деньги в БД в USDT
    if int(remains_money_user) > 0:
        database.delete_count_money(str(user_id), str(id_token))
        # удаляет старые токены
        database.load_to_wallet(str(user_id), str(id_token), str(remains_money_user))
